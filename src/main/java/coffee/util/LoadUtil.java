...
import coffee.resolver.RequestMappingResolver;
import coffee.xml.XmlEntity;
import coffee.xml.XmlParser;
...
public class LoadUtil {
	...
	public static void init(ServletConfig servletConfig){
		...
		try {
			...
			logger.info("coffee配置文件装载失败 失败原因可能是没有添加coffee.xml文件");
			parserResult = false;
		}
		if(parserResult) {
			logger.info("coffee配置文件装载成功");
